...
class TicketModelTest(TestCase):
    ...
    def setUp(self):
        # Create test objects for flight
    ...
    def test_seat_validation(self):
        ...
        with self.assertRaises(ValidationError):
            invalid_ticket = Ticket(
                ...
                row=50,  # Higher than max rows in airplane
                seat="A",
                status="booked"
            )
            invalid_ticket.full_clean()
        
        # Test invalid seat (out of range for airplane)
        with self.assertRaises(ValidationError):
            invalid_ticket = Ticket(
                flight=self.flight,
                passenger_name="Invalid Seat",
                row=10,
                seat="Z",  # Beyond available seats in row
                status="booked"
            )
            invalid_ticket.full_clean()
...
class TicketAPITest(APITestCase):
    ...
    def setUp(self):
        ...
        # Create order for the ticket (needed for permission checks)
    ...
    def test_user_can_view_ticket_details(self):
        ...
        # Verify flight details are included
        self.assertEqual(response.data['flight_details']['flight_number'], "SG321")

    def test_duplicate_seat_validation(self):
        """Test validation against booking already occupied seats"""
        self.client.force_authenticate(user=self.user)
        
        # Try to create a new ticket with the same seat
        duplicate_seat_data = {
            'flight': self.flight.id,
            'passenger_name': "Duplicate Seat User",
            'row': 20,  # Same row as existing ticket
            'seat': 'E',  # Same seat as existing ticket
            'status': 'booked'
        }
        ...
        # The error might be in non_field_errors due to unique_together constraint
        self.assertTrue(
